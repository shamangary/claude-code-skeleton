# MATCHING: skeleton ↔ `claude-code/`

Maps **`claude-code-skeleton/`** to **`claude-code/`** (paths relative to each repo root).

> **Generated** by `python3 matching/generate_matching.py`  
> Source of truth: YAML `matching.paths` on each **`MACRO.md`**.  
...
| Metric | Count |
|--------|-------:|
| Upstream Files Indexed | 183 |
| Macro 1 Many Paths | 149 |
| Macro 1 1 Pairs Total | 33 |
| Macro 1 1 Ok | 33 |
| Macro 1 1 Missing Upstream | 0 |
| Uncovered Upstream Files | 1 |
| Duplicate Claim Errors | 0 |
| Yaml Or Schema Errors | 0 |
| Missing Declared Upstream | 0 |

**Overall:** `PASS` — non-zero issue counts below mean the report is still written, but you should fix YAML or upstream sync.

## Ignored when indexing upstream

These path rules exclude files from **uncovered** / **indexed** counts (mirroring skeleton policy):

- Top-level only skipped: `['.git', '.vscode']`
- Any path segment skipped: `['.git']`
...
3. Paths in this doc are relative to each repo root.
...
## 1:many — `MACRO.md` → `matching.paths`
...
## Uncovered upstream files (informational)

Files under `claude-code/` not assigned to any `matching.paths`, not a 1:1 prompt pair, and not ignored by policy — you may add them to a `MACRO.md` or extend ignore rules.
